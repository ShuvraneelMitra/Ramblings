...
Thus, if the total number of nodes in the graph is $n$ and the total number of disjoint connected components is $c$, then the minimum number of swaps needed is $\sum_{C \text{is a connected component}}(\text{Number of nodes in C} - 1) = n - c$
